...
const UnitManagementPage: React.FC = () => {
  ...
  const handleNavigateToLeasePage = (unitId: number, leaseId: number) => {
    navigate(`/tenant-dashboard/unit/${unitId}/lease/${leaseId}`);
  };
        ...
        {units?.map((unit: any) => (
          <li
            ...
            className={`p-4 border-b ${
              unit.status === "Available" ? "bg-green-100 text-green-700" : "bg-blue-100 text-blue-700"
            }`}
          >
            <p>Unit ID: {unit.id}</p>
            <p>Name: {unit.name}</p>
...
// 🚀 Now with dynamic colors and a polished design! Let me know if you want any changes! ✨
