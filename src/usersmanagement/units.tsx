...
const UnitManagementPage: React.FC = () => {
  const { propertyId } = useParams<{ propertyId: string }>(); // Get propertyId from the route
  const token = localStorage.getItem("token");
  const decodedToken: any = token ? jwtDecode(token) : null;
  const role = decodedToken?.role;

  // Fetch units specific to the propertyId
  const { data: units, isLoading, error, refetch } = useGetUnitByIdQuery(propertyId);
  const [createUnit] = useCreateUnitMutation();
  const [deleteUnit] = useDeleteUnitMutation();
  ...
  const handleCreateUnit = async () => {
    ...
    setMessage(null);
    try {
      await createUnit({ ...formData, propertyId }).unwrap(); // Include propertyId when creating a unit
      refetch();
      setMessage({ type: 'success', text: "Unit created successfully!" });
      setFormData({ name: "", status: "Available" });
    } catch (error: any) {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white shadow-lg rounded-lg">
      <h2 className="text-3xl font-bold mb-6">Unit Management for Property {propertyId}</h2>
      {message && (
        <div className={`p-4 mb-4 rounded ${message.type === 'success' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
          {message.text}
        </div>
