...
const PaymentComponent: React.FC = () => {
  const { data: payments, isLoading, error, refetch } = useGetAllPaymentsQuery(undefined);
  const [createPayment] = useCreatePaymentMutation();
  const [deletePayment] = useDeletePaymentMutation();

  const [paymentId] = useState<number | "">("");
  ...
  useEffect(() => {
    if (singlePayment) {
      setAmountPaid(singlePayment.amount_paid.toString());
      setPaymentStatus(singlePayment.payment_status);
    }
  }, [singlePayment]);
