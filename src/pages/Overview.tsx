...
import React from "react";
import { Link } from "react-router-dom";
import { Users, Home, DollarSign, CheckCircle, Clock, TrendingUp, PieChart } from "lucide-react";
import { useGetAllUnitsQuery } from "../features/units/unitsAPI";
import { useGetAllLeasesQuery } from "../features/lease/leaseAPI";
import { useGetAllPaymentsQuery } from "../features/payments/paymentsAPI";
import { useGetAllTenantsQuery } from "../features/tenants/tenantsAPI";
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mb-6">
        <div className="bg-white p-6 shadow-md rounded-lg flex items-center hover:shadow-lg transition-shadow">
          <Home size={28} className="text-blue-500 mr-3" />
          <div>
            <p className="text-gray-500">Total Properties</p>
            <h2 className="text-xl font-semibold">{totalProperties}</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        ...
        <div className="bg-white p-6 rounded-lg shadow-md hover:shadow-lg transition-shadow">
          <h2 className="text-lg font-semibold mb-4 flex items-center">
            <TrendingUp size={20} className="mr-2 text-blue-500" /> Revenue Trend
          </h2>
          <ResponsiveContainer width="100%" height={300}>
            <ReBarChart data={revenueData}>
              <XAxis dataKey="month" />
        ...
        <div className="bg-white p-6 rounded-lg shadow-md hover:shadow-lg transition-shadow">
          <h2 className="text-lg font-semibold mb-4 flex items-center">
            <PieChart size={20} className="mr-2 text-purple-500" /> Unit Status Overview
          </h2>
          <ResponsiveContainer width="100%" height={300}>
            <RePieChart>
              <Pie data={unitStatusData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={100}>
      ...
      <div className="mt-6 bg-white p-6 rounded-lg shadow-md hover:shadow-lg transition-shadow">
        <h2 className="text-lg font-semibold mb-4 flex items-center">
          <Clock size={20} className="mr-2 text-gray-500" /> Recent Activity
        </h2>
        <ul className="space-y-3">
          {leases?.slice(0, 3).map((lease, index) => (
            <li key={index} className="flex items-center">
